...
void loop() {
  ...
  if(Serial.available() > 0) {
    ...
    if(val == 'x') {
      ignore = true;
      lights(true); // all lights on
    }
    if(val == 'y') {
      ignore = true;
      lights(false); // all lights off
    }
    if(val == 'z') {
      ignore = false;
      lights(false); // back to normal
    }
  } // end send Serial
