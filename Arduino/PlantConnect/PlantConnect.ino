 ...
 */
kSeries K30Sensor4(10, 2);      // software serial for CO2 sensor 4: RX=digital pin 10, TX=digital pin 2
kSeries K30Sensor5(11, 3);      // software serial for CO2 sensor 5: RX=digital pin 11, TX=digital pin 3
kSeries K30Sensor6(12, 4);      // software serial for CO2 sensor 6: RX=digital pin 12, TX=digital pin 4
kSeries K30Sensor7(13, 5);      // software serial for CO2 sensor 7: RX=digital pin 13, TX=digital pin 5
kSeries K30Sensor8(50, 6);      // software serial for CO2 sensor 8: RX=digital pin 50, TX=digital pin 6
kSeries K30Sensor9(51, 38);     // software serial for CO2 sensor 9: RX=digital pin 51, TX=digital pin 38
kSeries K30Sensor10(52, 39);    // software serial for CO2 sensor 10: RX=digital pin 52, TX=digital pin 39
kSeries K30Sensor11(53, 40);    // software serial for CO2 sensor 11: RX=digital pin 53, TX=digital pin 40
kSeries K30Sensor12(A8, 41);    // software serial for CO2 sensor 12: RX=digital pin A8, TX=digital pin 41
kSeries K30Sensor13(A9, 42);    // software serial for CO2 sensor 13: RX=digital pin A9, TX=digital pin 42
kSeries K30Sensor14(A10, 43);   // software serial for CO2 sensor 14: RX=digital pin A10, TX=digital pin 43
kSeries K30Sensor15(A11, 44);   // software serial for CO2 sensor 15: RX=digital pin A11, TX=digital pin 44
kSeries K30Sensor16(A12, 45);   // software serial for CO2 sensor 16: RX=digital pin A12, TX=digital pin 45
kSeries K30Sensor17(A13, 46);   // software serial for CO2 sensor 17: RX=digital pin A13, TX=digital pin 46

// commands to send to the K30 (using KSeries library)
const char PPM = 'p';
const char PERCENT = '%';
// command packet to read CO2 (using hardware serial)
...
const byte PROGMEM PPM_HW_SERIAL[] = {0xFE, 0x44, 0x00, 0x08, 0x02, 0x9F, 0x25};
...
void loop() {
  ...
  if(currMillis - prevMillis >= INTERVAL) {
    ...
    // get co2 value from K30Sensor4 to K30Sensor17 in ppm (parts per million) an store it
    K30Sensor4._Serial->listen();                          // K30Sensor4
    while(!K30Sensor4._Serial->isListening()) {;}
    co2Vals[3] = (uint32_t)K30Sensor4.getCO2(PPM);
    K30Sensor5._Serial->listen();                          // K30Sensor5
    while(!K30Sensor5._Serial->isListening()) {;}
    co2Vals[4] = (uint32_t)K30Sensor5.getCO2(PPM);
    K30Sensor6._Serial->listen();                          // K30Sensor6
    while(!K30Sensor6._Serial->isListening()) {;}
    co2Vals[5] = (uint32_t)K30Sensor6.getCO2(PPM);
    K30Sensor7._Serial->listen();                          // K30Sensor7
    while(!K30Sensor7._Serial->isListening()) {;}
    co2Vals[6] = (uint32_t)K30Sensor7.getCO2(PPM);
    K30Sensor8._Serial->listen();                          // K30Sensor8
    while(!K30Sensor8._Serial->isListening()) {;}
    co2Vals[7] = (uint32_t)K30Sensor8.getCO2(PPM);
    K30Sensor9._Serial->listen();                          // K30Sensor9
    while(!K30Sensor9._Serial->isListening()) {;}
    co2Vals[8] = (uint32_t)K30Sensor9.getCO2(PPM);
    K30Sensor10._Serial->listen();                         // K30Sensor10
    while(!K30Sensor10._Serial->isListening()) {;}
    co2Vals[9] = (uint32_t)K30Sensor10.getCO2(PPM);
    K30Sensor11._Serial->listen();                         // K30Sensor11
    while(!K30Sensor11._Serial->isListening()) {;}
    co2Vals[10] = (uint32_t)K30Sensor11.getCO2(PPM);
    K30Sensor12._Serial->listen();                         // K30Sensor12
    while(!K30Sensor12._Serial->isListening()) {;}
    co2Vals[11] = (uint32_t)K30Sensor12.getCO2(PPM);
    K30Sensor13._Serial->listen();                         // K30Sensor13
    while(!K30Sensor13._Serial->isListening()) {;}
    co2Vals[12] = (uint32_t)K30Sensor13.getCO2(PPM);
    K30Sensor14._Serial->listen();                         // K30Sensor14
    while(!K30Sensor14._Serial->isListening()) {;}
    co2Vals[13] = (uint32_t)K30Sensor14.getCO2(PPM);
    K30Sensor15._Serial->listen();                         // K30Sensor15
    while(!K30Sensor15._Serial->isListening()) {;}
    co2Vals[14] = (uint32_t)K30Sensor15.getCO2(PPM);
    K30Sensor16._Serial->listen();                         // K30Sensor16
    while(!K30Sensor16._Serial->isListening()) {;}
    co2Vals[15] = (uint32_t)K30Sensor16.getCO2(PPM);
    K30Sensor17._Serial->listen();                         // K30Sensor17
    while(!K30Sensor17._Serial->isListening()) {;}
    co2Vals[16] = (uint32_t)K30Sensor17.getCO2(PPM);
  } // end if
...
void sendRequest(uint8_t whichPort) {
  ...
  byte command[7];
  memcpy_P(&command, &PPM_HW_SERIAL, sizeof command);
